...
**4. 登录方式选择**
1.默认：AuthnHelper.LOGIN_TYPE_DEFAULT
当无中间件时，使用当前流量卡登录，有中间件时则使用中间件登录；
2.卡1：AuthnHelper.LOGIN_TYPE_SIM1
当无中间件时，选择卡1登录，当有中间件时，如果中间件是卡1的则使用中间件登录，否则清除中间件使用卡1登录；
3.卡2：AuthnHelper.LOGIN_TYPE_SIM2
当无中间件时，选择卡2登录，当有中间件时，如果中间件是卡2的则使用中间件登录，否则清除中间件使用卡2登录；
4.移动优先：AuthnHelper.LOGIN_TYPE_CMCC_FIRST
当无中间件时，
（1）如果当前流量卡是移动卡则使用当前移动卡登录；
（2）如果非数据流量卡是移动卡，则使用非数据流量卡的移动卡登录；
（3）如果没有移动卡，则使用当前流量卡登录；
有中间件时：
（1）如果中间件是移动卡，则使用中间件登录；
（2）如果中间件不是移动卡，且存在移动卡，则清除中间件使用移动卡登录，不存在移动卡，则默认使用中间件登录；
5.流量卡优先：
当无中间件时，使用当前数据流量卡登录，未开启数据流量时使用当前卡登录；
有中间件时，如果中间件是当前数据流量卡的中间件则使用中间件登录，否则清除中间件使用当前数据流量卡登录
# 2. SDK方法说明
